...
bool select_from_inner_join_on(const Command_line& line){
    if(Database_index==-1) {cout<<"Have not select database\n";return 0;}
    Parameter& para=line.parameter;
    Condition& con=para[para.size()-1];
    // //第一层：属于那个表
    // //第二层：这个表中被选中的row
    // //第三层指针
    // vector<vector<vector<Table_content>*>> table_content;
    Table table=convert_table_for_inner_join(get_table(para[1]));
    // table.Table_name="Select_from_inner_join_on"
    //加载第一个表

    for(Inner_join_on_config &i : get<Inner_join_on_configs>(para[2])){
    ...
    write_in(out,_select_from(table,line[0],con));
...
void inner_join(Table &table_tar,const Table & table_nex,const string & column[2]){
    ...
    for(int i=table_tar.row.size()-1;i>=0;i--){
        for(int e=table_nex.row.size()-1;e>=0;e--){
            if(table_tar.row[i][pos1]!=table_nex.row[e][pos2]) table_tar.row.erase(table_tar.row.begin()+i);
            else{
                for(auto & j : table_nex.row[e])
                table_tar.row[i].push_back(j);
            }
        }
    }
}
